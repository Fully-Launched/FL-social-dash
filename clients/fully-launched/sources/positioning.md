# Fully Launched — Your Voice

**The 3-3-3 framework: pillars, formats, perspectives**

**Last updated:** 2026-09-25
**Drive:** https://docs.google.com/document/d/1CzaSXHTk_25Qb6GoueDd_wxzHpAV0efVmD6REdf9k_4/edit (mirror of this file)
**Sources:** Tait's voice memo and notes in chat, and Luke Bothun's answers (2026-09-25). See Customer Data for the full recordings.
**Tags:** **[Tait]** / **[Luke]** mean said by them. **[To confirm]** means Tait hasn't decided yet.

---

## What this document is

Every client gets a **3-3-3**: **3 pillars** (what we always talk about), **3 formats** (the shapes our videos take) and **3 unique perspectives** (where we see things differently from everyone else). These are the categories we stay in. Every content idea should trace back to one pillar, one format and one perspective. **[Tait]**

**Fully Launched is the exception: 4 pillars instead of 3**, one for each service. **[Tait]**

---

## Pillars (4)

### 1. Websites
- **What we do:** redesign business websites to bring in new customers. **[Tait]**
- **Our angle:** custom websites that actually drive results and don't just look cool. Anyone can design a website with AI now; our strategy is making it simple, a simple, easy-to-use website. **[Tait]**

### 2. Organic content (social media)
- **What we do:** help businesses and founders bring in new clients and more cash flow through content on social media. **[Tait]**
- **Who it's for:** founders and business owners who spend all their time on the business, have no time to film, don't know what to post, and struggle to stay consistent. **[Tait]**
- **Our angle:** a whole content system in about 2 hours a month. One voice memo captures their ideas, we turn it into content ideas and a filming schedule, they film, and we post. **[Tait]**

### 3. AI systems
- **What we do:** build custom Claude solutions to business owners' problems. **[Tait]**
- **Our angle:** *"You're spending $60,000 a year on software that we can build you for $15,000, in a week, and you can have permanently."* **[Tait]** We take one painful manual process and ship a working solution in weeks, not months. **[Luke]**

### 4. Marketplaces (e-commerce)
- **What we do:** get people launched and selling on Shopify, Etsy, Amazon and TikTok Shop, and connect custom websites to Shopify. **[Tait]**
- **Who it's for:** the small business that wants to sell online but doesn't know how (the art shop), or the Etsy seller ready for their own Shopify store. **[Tait]**
- **Our angle:** listings that actually sell and stay profitable. More marketplaces don't mean more sales if the listing is bad. **[Luke]**

### Recurring series (runs across every pillar): Building Fully Launched
- **At least 4 videos a week** documenting the build of the business. It goes on the content calendar. **[Tait]**

---

## Formats (3)

1. **How to.** Teach the viewer to do the thing. **[Tait]**
2. **How we / How I.** Show how we do it for clients, or how Tait does it himself. **[Tait]**
3. **Behind the scenes.** Show customers what we're doing and how we can serve them. **[Tait]**

---

## Unique perspectives (3)

### 1. Make it simple
Everything we touch gets simplified: your website, your social media system, your business. The goal isn't just to make a website; it's to make a simple, easy-to-use website. Simplifying should be the thread through every category. **[Tait]**

### 2. Service media, not social media
When you post on social media, it should be *service media*. You're seeking to serve your customer with your content and help them know what to do next. Don't just give them information: give them an action step after they watch. **[Tait]** *(Tait plans to add more on how exactly we do this.)*

### 3. [To confirm] Start small, ship the one thing that works
Tait hasn't named the third perspective yet. **The strongest candidate is in Luke's own words:**
- One process automated, or one channel done right, beats a big transformational project that changes everything and is overwhelming. **[Luke]**
- Agencies love to sell big retainers and AI strategy instead of shipping the one thing that works. We start with one specific task or channel and measure it in hours saved or dollars kept. **[Luke]**

*Tait: keep this, change it, or replace it.*

---

## Where we post [Tait]

LinkedIn, Instagram, Facebook.

## Calls to action [Tait]

Every video points to one free tool that brings people onto the email list:
- Free store audit
- Free social media audit
- Free guide: how to build a social media system
- Free guide: 3 steps to simplify your website
- Questionnaire: how we can simplify your business and save you time with an AI system
- Free Etsy shop critique *(from the Reddit research)*

**Email:** a 9-email welcome sequence (coming soon).

## How to use this document

When building an idea, pick:
1. **One pillar**, or the Building Fully Launched series.
2. **One format.**
3. **One perspective.**
4. **One call to action.**

If an idea doesn't fit, it's probably not ours.
